...
const contactInfo = [];
for (i = 0; i < graduates.length; ++i) {
  grads = graduates[i].first_name + ' ' + graduates[i].email;
...
for (i = 0; i < zooAnimals.length; i++) {
  animal = `${zooAnimals[i].animal_name} ${zooAnimals[i].scientific_name}`;
...
let lower = function (zooAnimals) {
  let names = zooAnimals.animal_name.toLowerCase();
  // console.log(names);
  return names;

}
lowerCase= zooAnimals.map(lower);
